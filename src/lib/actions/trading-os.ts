...
export async function getOrCreateTradingDay(date: string): Promise<TradingDay | null> {
  ...
  const { data: created, error } = await db
    .from("trading_days")
    .insert({ user_id: userId, trade_date: date })
    .select()
    .single();
...
export async function savePreMarketChecklist(
  ...
  if (existing) {
    ...
    const { data, error } = await db
      .from("pre_market_checklists")
      .insert({ ...payload, user_id: userId, trading_day_id: dayId })
      .select()
      .single();
  ...
  const { data: day } = await db
    .from("trading_days").select("trade_date").eq("id", dayId).single();
...
export async function saveNoTradeFilter(
  ...
  await db.from("trading_days").update({
    status: newStatus,
...
export async function createNewsEvent(
  ...
  const { data: day } = await db.from("trading_days").select("trade_date").eq("id", dayId).single();
  if (day) revalidateDay((day as { trade_date: string }).trade_date);
  return data as NewsEvent;
}

export async function updateNewsEvent(id: string, payload: Partial<Omit<NewsEvent, "id" | "user_id" | "trading_day_id" | "created_at" | "updated_at">>) {
  const db = getDb();
  const { data, error } = await db.from("news_events").update(payload).eq("id", id).select().single();
...
export async function deleteNewsEvent(id: string) {
  const db = getDb();
  const { error } = await db.from("news_events").delete().eq("id", id);
  return !error;
}
...
export async function saveMarketAnalysis(
  ...
  const { data: existing } = await db
    ...
    .select("id").eq("trading_day_id", dayId).eq("timeframe", timeframe).single();

  let result;
  if (existing) {
    const { data, error } = await db
      .from("daily_market_analysis").update(payload).eq("id", existing.id).select().single();
    if (error) { console.error("[saveMarketAnalysis] update", error); return null; }
    result = data;
  } else {
    const { data, error } = await db
      .from("daily_market_analysis")
      .insert({ ...payload, user_id: userId, trading_day_id: dayId, timeframe })
      .select().single();
...
export async function savePsychologyLog(
  ...
  const { data: existing } = await db
    .from("psychology_logs").select("id").eq("trading_day_id", dayId).single();

  let result;
  if (existing) {
    const { data, error } = await db
      .from("psychology_logs").update(payload).eq("id", existing.id).select().single();
...
export async function createJournalTrade(
  ...
  const { count } = await db
    .from("journal_trades").select("*", { count: "exact", head: true })
    .eq("trading_day_id", dayId);
  ...
  await db.from("trading_days").update({ status: "trading" }).eq("id", dayId).eq("user_id", userId);
...
export async function updateJournalTrade(
  ...
  const db = getDb();
  const { data, error } = await db
    .from("journal_trades").update(payload).eq("id", id).select().single();
...
export async function deleteJournalTrade(id: string) {
  ...
  const { data: trade } = await db
    .from("journal_trades").select("trading_day_id, trade_date").eq("id", id).single();
  const { error } = await db.from("journal_trades").delete().eq("id", id);
  if (!error && trade) {
...
export async function addTradeManagementAction(
  ...
  const { data, error } = await db
    ...
    .insert({ ...payload, user_id: userId, trade_id: tradeId })
    .select().single();
...
export async function savePostTradeReview(
  ...
  const userId = getUserId();
  const { data: existing } = await db
    .from("post_trade_reviews").select("id").eq("trade_id", tradeId).single();

  let result;
  if (existing) {
    const { data, error } = await db
      .from("post_trade_reviews").update(payload).eq("id", existing.id).select().single();
...
export async function saveEodReview(
  ...
  const userId = getUserId();
  const { data: existing } = await db
    .from("eod_reviews").select("id").eq("trading_day_id", dayId).single();

  let result;
  if (existing) {
    const { data, error } = await db
      .from("eod_reviews").update(payload).eq("id", existing.id).select().single();
  ...
  await db.from("trading_days").update({ status: "session_complete" }).eq("id", dayId).eq("user_id", userId);
  const { data: day } = await db.from("trading_days").select("trade_date").eq("id", dayId).single();
  if (day) revalidateDay((day as { trade_date: string }).trade_date);
